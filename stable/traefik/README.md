...
**It is based on Helm community chart [traefik](https://github.com/kubernetes/charts/tree/master/stable/traefik)**
...
```bash
$ helm install tc/traefik --name traefik --namespace kube-system
```
...
```bash
$ kubectl describe service traefik-traefik -n kube-system | grep Ingress | awk '{print $3}'
```
...
$ helm install --name traefik --namespace kube-system \
  --set dashboard.enabled=true,dashboard.domain=traefik.example.com tc/traefik
```
...
```bash
$ helm install --name traefik --namespace kube-system --values values.yaml tc/traefik
```
