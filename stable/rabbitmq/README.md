...
**It is based on Helm community chart [rabbitmq](https://github.com/kubernetes/charts/tree/master/stable/rabbitmq)**
...
```bash
$ helm install --name my-release tc/rabbitmq
```
...
```bash
$ helm install --name my-release -f values.yaml tc/rabbitmq
```
...
```bash
$ helm install --name my-release -f values-production.yaml tc/rabbitmq
```
