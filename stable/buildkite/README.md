...
**Note:** Buildkite Agent [token](https://buildkite.com/docs/agent/configuration) must be set.
...
```bash
$ helm install tc/buildkite --name bk-agent --namespace buildkite --set agent.token="BUILDKITE_AGENT_TOKEN"
```

To install the chart with the release name `bk-agent` and set Agent meta-data and git repo SSH key:
```console
$ helm install tc/buildkite --name bk-agent --namespace buildkite \
